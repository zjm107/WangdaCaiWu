...
{
    public partial class FrmNDD_Word : Form
    {
        public FrmNDD_Word()
        {
            InitializeComponent();
        ...
        private void HyForm_Load(object sender, EventArgs e)
        {
            ...
            //绑定工具栏数据源
            SetDataSource(nDD_WORDBindingSource);
            //ComBoxListLoad();
        ...
        private void ComBoxListLoad()
        {
            try
            {
                var groups = GetTableFieldList("NDD_WORD", "GROUPNAME");
        ...
        private void simpleButton1_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (!Box.IsNullOrEmpty(mATERIALTYPETextBox.Text))
                    hashTable.Add("MATERIALTYPE", mATERIALTYPETextBox.Text.Trim());

                if (!Box.IsNullOrEmpty(mATERAILBREEDTextBox.Text))
                    hashTable.Add("MATERAILBREED", mATERAILBREEDTextBox.Text.Trim());

                if (!Box.IsNullOrEmpty(wORDTextBox.Text))
                    hashTable.Add("WORD", wORDTextBox.Text.Trim());

                dstNDD_Word.GetNDDWordData(hashTable);
                gridView1.BestFitColumns();
            }
            catch (Exception ex)
            {
